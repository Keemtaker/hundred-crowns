...
#   hash = response.parse
#   hash['businesses'].map do |b_hash|
#     { business_id: b_hash['id'], rating: b_hash['rating'] }
#   end
# end

# p search("pizza", "Norrebro, 2200 copenhagen N")
...
#
# Returns a parsed json object of the request
def review_for_business(business_id)
  url = "#{API_HOST}#{BUSINESS_PATH}#{business_id}#{REVIEWS}"

  response = HTTP.auth(bearer_token).get(url)
  hash = response.parse
  #iteration to get only reviews
  hash['reviews'].map do |review|
     { content: review['text'], user_name: review['user']['name'] }
  end

end
p review_for_business("pizzeria-la-fiorita-københavn")
